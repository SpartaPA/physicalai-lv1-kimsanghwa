...
class CoordinateChain:
    ...
    def _path_to_root(self, frame: str) -> list[str]:
        ...
        while cur != self.root:
            if cur not in self._parent:
                raise KeyError(f"'{frame}' 이(가) root('{self.root}') 에 연결되어 있지 않습니다.")
    ...
    def T_from_root(self, frame: str) -> np.ndarray:
        ...
        chain_frames = list(reversed(path))         # [root, ..., frame]

        T = np.eye(4)
        for parent, child in zip(chain_frames[:-1], chain_frames[1:]):
            T = T @ self.get(parent, child)
        return T
    ...
    def axis_angle(self, target: str, source: str):
        ...
        T = self.T(target, source)
        return axis_angle_from_matrix(T[:3, :3])
