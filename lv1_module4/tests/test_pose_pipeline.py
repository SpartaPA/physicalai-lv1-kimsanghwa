...
def test_camera_to_base_matches_chain(pipeline, rng):
    P = rng.uniform(-0.5, 0.5, size=(50, 3)) + [0, 0, 0.8]
    P_base = pipeline.camera_to_base(P)

    expected_chain = default_chain().transform("base", "camera", P)
    expected_matmul = transform_points(pipeline.T_base_link @ pipeline.T_link_camera, P)

    assert np.allclose(P_base, expected_chain)
    assert np.allclose(P_base, expected_matmul)
...
def test_roundtrip_restores_points(pipeline, rng):
    P = rng.uniform(-0.5, 0.5, size=(50, 3)) + [0, 0, 0.8]
    P_back = pipeline.base_to_camera(pipeline.camera_to_base(P))
